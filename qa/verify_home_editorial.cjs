...
(async () => {
  ...
  for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
      ...
      const result = await page.evaluate(() => ({
        ...
        coreTitle: [...document.querySelectorAll("h2")].find((node) => node.textContent.trim() === "甄选产品")?.textContent.trim(),
      ...
      assert(result.coreTitle === "甄选产品" && result.coreCards === 3, "zh curated selection changed unexpectedly");
      await page.close();
  }

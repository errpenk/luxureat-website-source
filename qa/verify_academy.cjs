...
assert(readerRuntime.includes("item.archive || item.eyebrow || item.topicLabel"), "related articles do not fall back to the indexed topic label");
assert(runtime.includes("data-reader-related") && runtime.includes("academyTriggerId"), "related academy articles do not load their full article chunk before opening");
assert(readerRuntime.includes('document.readyState === "loading"'), "a dynamically loaded reader can miss DOMContentLoaded and never initialize");

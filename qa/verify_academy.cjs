...
assert(titles.join("|") === expected.join("|"), `academy article order is wrong: ${titles.join("|")}`);
assert(Object.keys(data.articles).length === 18, "academy does not expose nine articles in both languages");
const zhTitles = data.order.map((slug) => data.articles[`zh-academy-${slug}`]?.title);
assert(zhTitles.every((title, index) => title && title !== expected[index]), "Chinese article titles are not localized");
...
for (const lang of ["zh", "en"]) {
  ...
  assert(html.includes('id="caviar-academy"') && html.includes("data-academy-list"), `${lang} Blog page is incomplete`);
...
assert(zhBlog.includes("知识、起源与工艺") && zhBlog.includes("<h2>鱼子酱学院</h2>"), "Chinese academy introduction is not localized");
assert(read("assets/js/core.js").includes('"鱼子酱学院", "caviar-academy"'), "Chinese Blog submenu is not localized");
const runtime = read("assets/js/academy.js");
assert(runtime.includes('search.addEventListener("input"') && runtime.includes("data-reader-open"), "academy search or reader integration is missing");
...
assert(styles.includes('--lux-zh-headline: "KingHwa Old Song Complete"'), "Chinese headings do not use the complete KingHwa font");

...
assert((read("zh/certification.html").match(/data-cert-award-open=/g) || []).length === 5 && integrationStyles.includes("flex-grow: 2.35") && integrationStyles.includes("figure:nth-child(-n + 2) { grid-column: span 3; }"), "five-image award accordion or mobile 2+3 leadership grid is missing");
assert(read("zh/certification.html").includes("集团旗下产品获奖记录") && (read("zh/certification.html").match(/lux-cert-brand-row/g) || []).length === 1 && integrationStyles.includes("height: 500px;"), "brand portfolio award heading, logo row or fixed-height accordion is missing");
...
assert(integrationStyles.includes("min-height: 500px;") && integrationStyles.includes("max-height: 500px;") && integrationStyles.includes("contain: layout paint;") && integrationStyles.includes("max-height: 390px;"), "award accordion height is not fully locked against image expansion");

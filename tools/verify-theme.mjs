...
assert(productDataJs.includes('const productImageOrder = [2, 1, 4, 3, 6, 7, 5, 8, 9, 11, 10, 13, 12, 16, 14, 15, 18, 19, 20, 17, 23, 21, 22, 24]') && productDataJs.includes('priceLabel: "PRICE"'), 'Appennino catalogue maps 24 Italian-labelled mockups by product and uses PRICE labels');
...
assert(['caviar', 'truffle', 'olive-oil'].every((category) => zhHome.includes(`luxureat_static_url('zh/product', '?category=${category}#product-catalogue')`)), 'Chinese home shop CTAs open all three product series');
assert(['caviar', 'truffle', 'olive-oil'].every((category) => enHome.includes(`luxureat_static_url('en/product', '?category=${category}#product-catalogue')`)), 'English home shop CTAs open all three product series');

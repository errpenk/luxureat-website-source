...
function fontPreloads(page) {
  ...
  const versionFor = (font) => ["KingHwaOldSong-hero-critical.woff2", "KingHwaOldSong-home-critical.woff2", "LuxurEatZhiSong-hero-critical.woff2"].includes(font) ? `${assetVersion}-home-font5` : assetVersion;
  const preloadFonts = fonts.filter(([, , , , preload = true]) => preload).sort(([font]) => font.includes("hero-critical") ? -1 : 0);
  const links = [
  ...
  const headlineFonts = '"KingHwa Page Critical","KingHwa Old Song Site","Songti SC","STSong",serif';
  const bodyFonts = '"ZhiSong Page Critical","LuxurEat ZhiSong Site","Songti SC","STSong",serif';
  const localeFonts = page.lang === "zh" ? `html[lang^="zh"]{--lux-page-heading:${headlineFonts}!important;--lux-zh-headline:${headlineFonts}!important;--lux-zh-body:${bodyFonts}!important}${page.key === "home" ? 'html[lang^="zh"] .lux-home-hero .zh-display-title{font-family:"KingHwa Hero Critical","KingHwa Page Critical","KingHwa Old Song Site","Songti SC","STSong",serif!important}html[lang^="zh"] body :is(.lux-header,.lux-home-hero,.lux-cookie-banner) :is(p,a,span,button){font-family:"ZhiSong Hero Critical","ZhiSong Page Critical","LuxurEat ZhiSong Site","Songti SC","STSong",serif!important}' : ""}` : "";
